...
class SankeyTransferPattern(BasePattern):
    """
    Cross-domain flow (Sankey semantics).
    """

    kind = PatternKind.SANKEY_TRANSFER
    ...
    def affordances(self) -> Tuple[str, ...]:
        return ("flow", "width_value", "uncertainty_alpha")

    def build(self, *, frame_payload: Mapping[str, Any], seed: int) -> PatternBuildResult:
        flows = frame_payload.get("flows")
        if not isinstance(flows, list) or not flows:
            inst = self._instance(
                pattern_id="sankey_transfer/v1", inputs={"flows": flows}, failure_mode="no_flows"
            )
